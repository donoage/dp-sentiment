...
class EODScheduler {
  ...
  // Check if it's time to run EOD snapshot (4:30 PM ET on weekdays)
  shouldRunEODSnapshot() {
    ...
    // Check if it's 4:30 PM ET
    const hours = etTime.getHours();
    const minutes = etTime.getMinutes();
    
    // Run at 4:30 PM ET (16:30)
  ...
  // Start the scheduler
  start() {
    console.log('📅 EOD Scheduler started - will snapshot at 4:30 PM ET on weekdays');
  ...
  async runEODSnapshot() {
    try {
      console.log('🔔 Running EOD snapshot at 4:30 PM ET...');
