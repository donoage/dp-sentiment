...
class IntradayChart {
  ...
  async fetchData(date = null) {
    try {
      const url = date ? `/api/intraday-snapshots?date=${date}` : `/api/intraday-snapshots`;
      ...
      if (!date) {
        ...
        this.data = snapshots
          .filter(s => {
            const { DateTime } = luxon;
            const snapshotDT = DateTime.fromISO(s.snapshot_time, { zone: 'utc' }).setZone(
              'America/New_York'
            );
            ...
            const etDT = DateTime.fromISO(s.snapshot_time, { zone: 'utc' }).setZone(
              'America/New_York'
            );
  ...
  drawEmptyState() {
    ...
    this.ctx.fillText(
      'Data is captured every 5 minutes during market hours',
      width / 2,
      height / 2 + 25
    );
  }
